...
## Phase 6B-2 잭잭 버거 스타일 교정

2026-08-26 확인 결과, Phase 6B-2 로컬 승인 근거에서 잭잭의 `approvedStyle`이
`unclassified`로 기록되어 있었다. 승인 적용기는 이 값을 게시 검수표의
`burgerStyle`로 그대로 복사했고, 게시 SQL 생성기도 해당 값을 그대로 직렬화했다.
따라서 원격 조회나 Flutter 매핑 문제가 아니라 로컬 승인 원천값 오류가 근본 원인이다.

로컬 승인 원천은 `chicken`으로 교정하고 이전 값 `unclassified`를 명시적인 교정
사전 조건으로 보존했다. 게시 검수표에서는 잭잭 한 행의 `burgerStyle`만 교정했으며
UUID, 이름, 현재 주소, 좌표, 게시 결정과 활성 상태는 변경하지 않았다. 과거에 적용된
2행 INSERT SQL은 당시 실행 이력으로 보존하고 다시 실행하거나 덮어쓰지 않는다.

별도 1행 UPDATE SQL은 실행 전 잭잭의 UUID·이름·현재 주소·`verified + active` 상태,
현재 `unclassified`, 공개 매장 25곳을 모두 검사한다. UPDATE 후 `chicken`과 공개 매장
25곳을 다시 검사한다. 사용자는 이 SQL을 원격 Supabase에 적용했고, 교정 후에도
`verified + active` 공개 매장 25곳이 유지되는 것을 확인했다. 실제 앱의 치킨 필터에서
잭잭이 표시되는 것도 확인했다. 이번 마감 작업에서는 원격 Supabase를 추가 변경하지
않았다.
