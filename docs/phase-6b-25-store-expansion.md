...
기준일은 2026-08-26이다. 원격 Supabase의 현재 `verified + active`는 23곳이며,
사용자는 스태커버거샵과 잭잭 두 곳을 명시적으로 승인했다. 아메리칸치즈버거는 동명
...
이 문서 시점에는 원격 SQL을 실행하지 않았다. 따라서 현재 원격 공개 매장은 계속
23곳이고, 사용자 승인 SQL을 한 번 수동 적용해 모든 검증이 통과한 뒤에만 25곳이 될
예정이다. 이는 로컬 승인과 예상치이며 원격 25곳 달성 완료를 뜻하지 않는다.
